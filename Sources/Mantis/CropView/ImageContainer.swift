...
final class ImageContainer: UIView {
    ...
    init(image: CIImage) {
        guard let r = CIImageRenderer.create() else {
            fatalError("Metal is unavailable on this device")
        }
        self.renderer = r
        super.init(frame: .zero)

        renderer.setUsingHighDynamicRange()
        renderer.displayedCIImage = image

        let metalView = r.mtkView
